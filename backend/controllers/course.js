...
exports.getFullCourseDetails = async (req, res) => {
    try {
        ...
        const isStudent = Boolean(req.user.accountType === "Student");
        ...
        const _courseContent = [...courseDetails.courseContent];

        if (isStudent) {
            const _newCourseContent = await Promise.all(
                _courseContent.map(async (section) => {
                    const subSections = [...section.subSection];

                    const newSubSections = await Promise.all(
                        subSections.map(async (item) => {
            ...
            courseDetails = {
                ...courseDetails,
                courseContent: _newCourseContent,
            };
        }
    ...
    try {
        ...
        for (const courseId of coursesId) {
            ...
            if (!course) {
                return res
                    .status(404)
                    .json({ success: false, message: `Course with ID ${courseId} not found` });
            }
            ...
            if (course.studentsEnrolled.includes(userId)) {
                return res.status(400).json({
                    success: false,
                    message: `You are already enrolled in the course ${course.courseName}`,
                });
            }
            ...
            // Add the course and progress to the user's data
        ...
        return res.status(200).json({
            success: true,
            message: "Successfully enrolled in the selected courses",
        });
    } catch (error) {
        console.error(error);
        return res
            .status(500)
            .json({ success: false, message: "An error occurred while enrolling in the courses" });
    }
};
